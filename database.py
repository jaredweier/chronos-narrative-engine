...
def get_all_evidence_file_categories() -> List[str]:
    with get_db_connection() as conn:
        rows = conn.execute("SELECT DISTINCT category FROM evidence_files ORDER BY category").fetchall()
...
def get_dashboard_analytics(days: int = 30) -> Dict[str, Any]:
    ...
    with get_db_connection() as conn:
        ...
        evidence_count = conn.execute("SELECT COUNT(*) as c FROM evidence_files").fetchone()['c']
        return {
            "total_reports": total,
...
def get_login_audit_logs(limit: int = 100, badge_id: str = "") -> List[Dict[str, Any]]:
    with get_db_connection() as conn:
        if badge_id:
            ...
            rows = conn.execute(
                "SELECT * FROM login_audit_log ORDER BY attempt_time DESC LIMIT ?", (limit,)
            ).fetchall()
        return [dict(r) for r in rows]
