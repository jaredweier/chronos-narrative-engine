...
REDACTION_CATEGORIES = {
    ...
    "name": {
        ...
        "flags": re.IGNORECASE,
        "patterns": [
            r'\b(?:Officer|Det\.|Detective|Sgt\.|Sergeant|Lt\.|Lieutenant|Cpl\.|Corporal|Cpt\.|Captain|Chief|Deputy|Agent|Inspector|Trooper)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\b(?:Suspect|Victim|Witness|Complainant|Reporting Party|Subject|Involved Party)\s*[:,]?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\b(?:Mr\.|Mrs\.|Ms\.|Dr\.)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\bContact\s*[:,]?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\b(?:Name|Name of (?:Suspect|Victim|Witness|Complainant|Subject))\s*[:,]\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\b(?:Male|Female)\s+(?:Subject|Suspect|Victim|Witness)\s*[:,]?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\b(?:stated that|contacted|registered to|identified as|spoke with|said|told|arrested|detained|interviewed|canvassed)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            r'\b(?:vehicle registered to|registered owner)\s*[:,]?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
        ],
